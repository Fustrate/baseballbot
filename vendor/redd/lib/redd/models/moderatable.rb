...
module Redd
  module Models
    ...
    module Moderatable
      ...
      def approve = @client.post('/api/approve', id: get_attribute(:name))

      # Remove a submission.
      # @param spam [Boolean] whether or not this item is removed due to it being spam
      def remove(spam: false) = @client.post('/api/remove', id: get_attribute(:name), spam:)
      ...
      def distinguish(how = :yes)
        params = { id: get_attribute(:name), how: }
      ...
      def undistinguish = distinguish(:no)

      # Stop getting any moderator-related reports on the thing.
      def ignore_reports = @client.post('/api/ignore_reports', id: get_attribute(:name))

      # Start getting moderator-related reports on the thing again.
      def unignore_reports = @client.post('/api/unignore_reports', id: get_attribute(:name))
    end
  end
end
