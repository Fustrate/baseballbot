  ...
  module Templates
    module Blocks
      class Calendar < Block
        def render
          <<~MARKDOWN
            #{interpolate(attributes['title'] || '## {{month_name}} Schedule')}
        ...
        def month_schedule
          @month_schedule ||= SubredditSchedule.new(subreddit:).generate(month_start, month_end)
        end
        ...
        class SubredditSchedule
          ...
          def initialize(subreddit:, team_id: nil)
            @subreddit = subreddit
            @team_id = team_id || subreddit.team&.id
          end
